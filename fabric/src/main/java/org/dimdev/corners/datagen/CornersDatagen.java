    ...
    public void onInitializeDataGenerator(FabricDataGenerator fabricDataGenerator) {
        ...
        pack.addProvider((FabricDataGenerator.Pack.RegistryDependentFactory<DataProvider>) (output, registriesFuture) -> new FabricDynamicRegistryProvider(output, registriesFuture) {
            ...
            protected void configure(HolderLookup.Provider registries, Entries entries) {
                ...
                registries.lookup(LimLibRegistryKeys.SKYBOX).ifPresent(entries::addAll);
                registries.lookup(LimLibRegistryKeys.DIMENSION_EFFECTS).ifPresent(entries::addAll);
                registries.lookup(LimLibRegistryKeys.POST_EFFECT).ifPresent(entries::addAll);
            ...
            public DataProvider create(FabricDataOutput output, CompletableFuture<HolderLookup.Provider> registriesFuture) {
                return new FabricCodecDataProvider<>(output, registriesFuture, Registries.LEVEL_STEM, LevelStem.CODEC) {
                    ...
                    protected void configure(BiConsumer<ResourceLocation, LevelStem> provider, HolderLookup.Provider lookup) {
                        lookup.lookup(Registries.LEVEL_STEM).stream().flatMap(HolderLookup::listElements).forEach(new Consumer<Holder.Reference<LevelStem>>() {
                            @Override
...
//                        .build(TheCorners.id("root")));
    ...
    public void buildRegistry(RegistrySetBuilder builder) {
        ...
        builder.add(LimLibRegistryKeys.SKYBOX, CornersDynamicRegistryProvider::skyboxes);
        builder.add(LimLibRegistryKeys.DIMENSION_EFFECTS, CornersDynamicRegistryProvider::dimensionEffects);
        builder.add(LimLibRegistryKeys.POST_EFFECT, CornersDynamicRegistryProvider::postEffects);
